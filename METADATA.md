...
| `label` | A representative label for the issuer where name maybe too long or misrepresentative |
| `currently_issuing` | Boolean variable that indicates if an issuer is currently issuing. Assumed issuer is actively issuing if not included in metadata. |
